...
enum class TimeUnits(val ms: Int, val russianCases: Array<String>) {
    ...
    fun plural(value: Long): String {
        ...
        return "${value.absoluteValue/this.ms} ${this.russianCases[case]}"
    }
...
fun Date.humanizeDiff(date: Date = Date()) =
    when(val diff: Long = date.time - time) {
        ...
        in 75.sec..45.min -> "${TimeUnits.MINUTE.plural(diff)} назад"
        in 45.min..75.min-> "час назад"
        in 75.min .. 22.hour -> "${TimeUnits.HOUR.plural(diff)} назад"
        in 22.hour .. 26.hour -> "день назад"
        in 27.hour..360L.day -> "${TimeUnits.DAY.plural(diff)} назад"
        in (-1).sec..0.sec -> "прямо сейчас"
        in (-45).sec..(-1).sec -> "через несколько секунд"
        in (-75).sec..(-45).sec -> "через минуту"
        in (-45).min..(-75).sec -> "через ${TimeUnits.MINUTE.plural(diff)}"
        in (-75).min..(-45).min -> "через час"
        in (-22).hour..(-75).min -> "через ${TimeUnits.HOUR.plural(diff)}"
        in (-26).hour..(-22).hour -> "через день"
        in (-360L).day..(-26).hour -> "через ${TimeUnits.DAY.plural(diff)}"
